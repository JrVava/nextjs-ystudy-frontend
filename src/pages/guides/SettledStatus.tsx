...
export default async function SettledStatus() {
  const data = await getCMSPageContent("settled-status");

  if (!data) {
    return (
      <div style={{ padding: "4rem", textAlign: "center", color: "var(--muted)" }}>
        <h2>Guide Not Found</h2>
        <p>We couldn't retrieve the settled status guide details at this time.</p>
  ...
  return (
    <div className="qualification-page settled-status-page">
      {/* HERO BANNER */}
      <Banner
        slug="settled-status"
        fallbackBadgeText={s1.badge || "Guide · Eligibility"}
        fallbackTitle={s1.title || "Settled status and Student Finance"}
        fallbackDescription={s1.description || "Understand how settled status, pre-settled status and residency history may affect funding routes."}
