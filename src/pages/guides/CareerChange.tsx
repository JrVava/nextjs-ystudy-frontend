...
export default async function CareerChange() {
  const [data, guides] = await Promise.all([
    getCMSPageContent("career-change"),
    getGuidesList()
  ]);
  ...
  if (!data) {
    return (
      <div style={{ padding: "4rem", textAlign: "center", color: "var(--muted)" }}>
        <h2>Career Change Page Not Found</h2>
        <p>We couldn't retrieve the career change guides at this time.</p>
  ...
  return (
    <div className="qualification-page career-change-page">
      ...
      <Banner
        slug="career-change"
        fallbackBadgeText={s1.badge || "Career Change Guide"}
        fallbackTitle={s1.title || "Change career through study."}
        fallbackDescription={s1.description || "Compare realistic study routes for adults in their 30s, 40s and 50s."}
        fallbackBgImage={s1.bgImage || "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=2200&q=80"}
        fallbackRightCard={{
          layoutType: "grid-2x2",
          title: "Snapshot",
          items: [
        ...
        <section className="conv-section">
          <div className="container">
            <div className="conv-head">
              ...
              <h2>{s2.title || "Career Change Guides"}</h2>
            ...
            <div className="guide-grid guide-carousel">
              {(guides && guides.length > 0 ? guides : (s2.cards || [])).map((card: any, idx: number) => (
      ...
      {/* PERSONAL ROUTE ADVISER CALL CTA */}
      {s3.status !== false && (
        <section className="conv-section">
          <div className="container">
